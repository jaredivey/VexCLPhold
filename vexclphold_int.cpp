...
const unsigned int num_events = 2 * num_lps;
const unsigned int stop_time = 6000000;

const unsigned int block_size = 128;
const unsigned int grid_size = ((num_events + block_size - 1) / block_size);
const unsigned int grid_run_size = ((num_lps + block_size - 1) / block_size);
...
int main(int argc, char** argv)
{
	...
	vex::vector<unsigned int> d_event_time (ctx, num_events);
	vex::vector<float> d_remote_flip (ctx, num_lps);
	d_event_time = random_state_int (0, (1337 << 20) + vex::element_index()) % 1000000;

	vex::vector<unsigned int> d_events_processed (ctx, num_lps);
	d_events_processed = 0;
	vex::vector<unsigned int> d_lp_current_time (ctx, num_lps);
	d_lp_current_time = 0;
	...
	vex::vector<unsigned int> d_current_lbts (ctx, 1);

	unsigned int  current_lbts;
	...
	simKernel.emplace_back(ctx.queue(0),
		VEX_STRINGIZE_SOURCE(
				kernel void simulatorRun(global unsigned int *current_time,
					global unsigned int *event_time,
					global unsigned int *event_lp,
					global unsigned int *current_lbps,
					...
					global unsigned int *events_processed)
	{
		...
		const unsigned int delay_time = 90000;
		const unsigned  int look_ahead = 400000;

		if (idx < num_lps)
		{
			unsigned int safe_time = *current_lbps + look_ahead;

			// Check the next event
			unsigned int next_event_time = event_time[idx];

			// Ok to process?
			if(next_event_time <= safe_time && next_event_time < stop_time)
			{
				unsigned int cur_time = current_time[idx];
				...
				unsigned int new_event_time = delay_time + next_event_time;
	...
	vex::Reductor<unsigned int, vex::MIN> min(ctx);
