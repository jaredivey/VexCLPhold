...
int main(int argc, char** argv)
{
	...
	std::vector<vex::backend::kernel> markWndKernel;
						...
						global unsigned char *next_event_flag)
	{
		...
		const float safe_time = *current_lbts + look_ahead;
		unsigned char flag;
					...
					global unsigned int *events_processed)
	{
		...
		const float rmt_flip = remote_flip[idx];
		const unsigned int tgt_lp = target_lp[idx];
		...
		if (idx < num_lps)
		{
			const float safe_time = *current_lbps + look_ahead;
